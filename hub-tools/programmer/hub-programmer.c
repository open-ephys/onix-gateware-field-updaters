...
*
* This software reports current hardware and firmware information about a ONIX hub and, optionally, writes a
* file prepared by bf_prep to an applicable FPGA/flash system.
*
* Usage:
* hub_programmer <driver> <slot> <hub index> [voltage [binary_image [-y]]]
*
...
* [voltage [binary_image [-y]]]: Optional arguments
*   - voltage: Voltage, in volts, to apply to the port prior to programming. This only functions for hubs 1
*     and 2 and will throw an error otherwise. This option is required for programming hubs 1 or 2.
*   - [binary_image [-y]]: Optional arguments required for writing firmware update file
*       - binary_image: File generated by bf_prep. If not provided, the program will print information about
*         the specified hub.
*       - [-y]: If specified, override the "are you sure" prompt
*
* Examples:
* `hub_programmer riffa 0 0`
*   - Examine hub 0 hardware and firmware information
* `hub_programmer riffa 0 2 5.0`
*   - Apply 5.0 volts to port 2 and examine hub 2 hardware and firmware information
* `hub_programmer riffa 0 2 6.0 headstage_rhs2116.onix`
*    - Apply 6.0 volts to port 2 and write headstage_rhs2116.onix update file
********************************************************/
...
static void read_data(int fd_in, char* dst, size_t size)
{
    size_t toread = size;
    size_t bytes_read = 0;
    int rc;
...
int main (int argc, char*argv[])
{
    ...
    if (argc < 4 || argc > 7)
    {
        printf("Usage: %s <driver> <slot> <hub index> [voltage binary_image] [-y]\n", argv[0]);
        exit(-1);
    }
    ...
    if (argc >= 5)
    {
        if (hub != 1 && hub != 2)
        {
            printf("Error: a port voltage can only be specified for hubs 1 and 2.\n");
            exit(-1);
        }

        double voltage = atof(argv[4]);
        if (voltage < 3.3 || voltage > 8.0) {
            printf("Error: specified voltage, %f V, is out of allowable range, 3.3-8.0V\n", voltage);
            exit(-1);
        }

        oni_write_reg(ctx, hub, 3, (unsigned int)(voltage * 10)); // Port control device address happens to match the port on ONIX
#ifdef _WIN32
        Sleep(500);
#else
        sleep(1);
    ...
    if (!noprog)
    {
        fd_in = open(argv[5], O_RDONLY | O_BINARY);
        if (fd_in < 0)
        {
